    ...
    <div class="card">
      ...
      <form v-else class="form" @submit.prevent="handleDownload">
        ...
        <div class="panel-actions">
          ...
          >
            {{ exporting ? t("admin.exports.downloading") : t("admin.exports.download") }}
          </button>
        </div>
      </form>
...
const buildFilename = (name: string, fromValue: string, toValue: string) => {
  ...
  return `pontos_${safeName || "colaborador"}_${period}.csv`;
};
...
const handleDownload = async () => {
  ...
  try {
    ...
    const blob = await api.getBlob(
      `/admin/exports/employees/${selectedEmployeeId.value}.csv?${query.toString()}`,
    );
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.setAttribute("download", buildFilename(selectedEmployee.value.fullName, from.value, to.value));
